...
externalNameHyperlink (srcs, _, xmap) name content = case Map.lookup mdl srcs of
    Just SrcLocal -> Html.anchor content !
        [ Html.href $ hypSrcModuleNameUrl mdl name, Html.strAttr "data-xdef" xdef ]
    Just (SrcExternal path) -> Html.anchor content !
        [ Html.href $ path </> hypSrcModuleNameUrl mdl name, Html.strAttr "data-xdef" xdef ]
    Nothing -> let mdlname = GHC.moduleNameString (GHC.moduleName mdl)
               ...
               in Html.anchor content ! [ Html.strAttr "data-xdef" xdef, Html.strAttr "data-xref" xref ]
  where
    mdl = GHC.nameModule name
    xdef = case Map.lookup name xmap of
             Just (GHC.L _ imp) -> GHC.moduleNameString (GHC.unLoc (GHC.ideclName imp))
             Nothing            -> ""
